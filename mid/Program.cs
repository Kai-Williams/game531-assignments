...
namespace midtermGame
{
    ...
    public class Game : GameWindow
    {
        ...
        protected override void OnLoad()
        {
            ...
            // --- Load shaders from Shaders/ beside the EXE ---
            _shader = Shader.FromFiles(); // defaults to "vert.glsl" + "frag.glsl"

            // Helper to build absolute image paths beside the EXE
            static string Img(string name) =>
                Path.Combine(AppContext.BaseDirectory, "Images", name);

            // --- Skybox setup ---
            _skyCube = Mesh.CreateSkyCube(1f);
            _skyTex = Texture.Load2D(Img("8k_stars_milky_way.jpg"));

            // --- Load textures for all planets (use SRGB for color/lighting correctness) ---
            var sunTex = Texture.Load2D(Img("8k_sun.jpg"), srgb: true);
            var earthTex = Texture.Load2D(Img("8k_earth_daymap.jpg"), srgb: true);
            var moonTex = Texture.Load2D(Img("8k_moon.jpg"), srgb: true);
            var marsTex = Texture.Load2D(Img("8k_mars.jpg"), srgb: true);
            var jupiterTex = Texture.Load2D(Img("8k_jupiter.jpg"), srgb: true);
            var saturnTex = Texture.Load2D(Img("8k_saturn.jpg"), srgb: true);
            var venusTex = Texture.Load2D(Img("4k_venus_atmosphere.jpg"), srgb: true);
            var uranusTex = Texture.Load2D(Img("2k_uranus.jpg"), srgb: true);
            var neptuneTex = Texture.Load2D(Img("2k_neptune.jpg"), srgb: true);
            ...
            // --- Rings (unlit alpha) ---
            _saturnRingTex = Texture.Load2D(Img("saturn_rings.png"));
